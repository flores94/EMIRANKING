...
window.onload = function () {
    ...
    if (!registro) {
        ...
        document.querySelector(".game-ready").style.display = "";
    } else {
        document.getElementById("registroPopup").style.display = "";
...
function startGame() {
    ...
    // Colocá acá la lógica real de tu juego
}
...
var Game = /** @class */ (function () {
    ...
    function guardarPuntaje(puntaje) {
        const registro = JSON.parse(localStorage.getItem("registroUsuario"));
        if (!registro) return;
        ...
        let ranking = JSON.parse(localStorage.getItem("rankingUsuarios")) || [];
        ...
        localStorage.setItem("rankingUsuarios", JSON.stringify(ranking));
    }
    Game.prototype.endGame = function () {
        this.updateState(this.STATES.ENDED);
        const puntajeFinal = this.blocks.length - 1;
        guardarPuntaje(puntajeFinal);
        mostrarPopupRanking(); // Esta función la explico abajo
    };
    function mostrarPopupRanking() {
        const ranking = JSON.parse(localStorage.getItem("rankingUsuarios")) || [];
        const lista = document.getElementById("rankingLista");
        lista.innerHTML = "";
    
        ranking.forEach((item, index) => {
            const li = document.createElement("li");
            li.textContent = `${index + 1}. ${item.nombre} ${item.apellido} - ${item.puntaje}`;
            lista.appendChild(li);
        });
    
        document.getElementById("popupRanking").style.display = "block";
    }
